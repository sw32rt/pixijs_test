...
export class Path {
    ...
    drawPath() {
        ...
        //     prevΘ = Θ
        // }
        const scale = { x: 1 / RESOLUTION, y: 1 / RESOLUTION };
    ...
    getClosestPointInfo(cursor) {
        ...
        if ((closest == undefined) || (adjacent == undefined)) {
            return;
        }
        ...
        const scaledVector = { x: (perpendiculerPoint.x - cursor.x), y: (perpendiculerPoint.y - cursor.y) };
        scaledVector.x *= 2;
        scaledVector.y *= 2;
        const mirrorPoint = {
    ...
    searchClosestPoints(cursor) {
        ...
        const nextA = this.divpath[shortestPoint.index + 1];
        const prevA = this.divpath[shortestPoint.index - 1];
    ...
    getPointByDistance(distanceFromStart) {
        ...
        const prevToNextVector = { x: (nextWayPoint.x - prevWayPoint.x), y: (nextWayPoint.y - prevWayPoint.y) };
