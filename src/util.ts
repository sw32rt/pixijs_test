...
export function judgeIentersected(ax: number, ay: number, bx: number, by: number, cx: number, cy: number, dx: number, dy: number): boolean {
...
export function getPerpendicular(x1: number, y1: number, x2: number, y2: number, px: number, py: number): { x: number, y: number } {
  // 線分Wの単位ベクトルUを求める
